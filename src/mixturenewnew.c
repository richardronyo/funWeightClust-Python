/*
This C library file has been extracted from the R package mixture - Mixture Models for Clustering and Classification
Version: 1.4
Date: 2015-03-10
...
int g, i, n;
    ...
    double mmtol = *mtol;
    int mmmax = *mmax;
    double *D = (double*)malloc(sizeof(double)*q*q);

    double **Sigma      = malloc(sizeof(double*)*G);
    double **gama      = malloc(sizeof(double*)*G);
    double **invSigma   = malloc(sizeof(double*)*G);
    double **sampcov    = malloc(sizeof(double*)*G);

    for(int g=0; g < G; g++) {
        Sigma[g]        = (double*)malloc(sizeof(double)*q*q);
        invSigma[g]     = (double*)malloc(sizeof(double)*q*q);
	   gama[g]     = (double*)malloc(sizeof(double)*q*p);
        sampcov[g]      = (double*)malloc(sizeof(double)*q*q);
    }
    //Rprintf("Allocated Memory\n");
//double *pi = malloc(sizeof(double)*G);
    ...
    for(int i=0; i<q; i++)
     ...
     //Rprintf("Initializing main loop G=%i\n", G);
     for(int g = 0; g < G; g++)
	{
	//Rprintf("Reading g=%i\n", g);
        Gam1(NN,pp,qq,GG,x,y,t,&g,gama[g]);
        //Rprintf("Wrote into gama for g=%i\n", g);
	CovarianceY(NN,pp,qq,GG,x,y,t,gama[g],&g, sampcov[g]);
	//Rprintf("Wrote into sampcov for g=%i\n", g);

	}

      // get_pi(N, G, t, pi);
      // error occurs in here (sampcov is qxq should be pxp)
       modeltype(q, pi, G, D, sampcov, Sigma, invSigma, logi, mmtol, mmmax, modely);
       //Rprintf("Ran modeltype\n");
...
for(g=0; g<G; g++){
       for(i=0; i<q*q; i++){
           covyi[g*q*q +i] = Sigma[g][i];
           icovyi[g*q*q +i] = invSigma[g][i];

        }
     for(i=0; i<q*p; i++)
	gami[g*q*p +i] = gama[g][i];
     }
     //Rprintf("Wrote into R variables\n");
   ...
   // Rprintf("sum=%e\n",sum);
     ...
     for(g=0; g<G; g++) {
         for(i=0; i<p; i++) {
           ...
           //   Rprintf("cluster=%i,Sigma[%i][%i]=%e\n", g,g, i, Sigma[g][i*p + i]);
         }
     }
       for(g=0; g<G; g++){
         logdet[g] = p*log(lam);
        // Rprintf("cluster=%i, loglik=%e",g,logdet[g]);
       }
       free(W);
}
...
void C_rmahalanobis(int *NN, int *pp,int *qq,int *GG, int *gg, double *x,double *y, double *gam, double *cov, double *delta)
{
    ...
    double *tX = (double*)malloc(sizeof(double)*N*p);

    double *aux1      = (double*)malloc(sizeof(double)*q*N);

    //Rprintf("Allocated Memory\n");
    double sum, insum;
    transpose(x,NN,pp,tX);
// aux1 is q XN
    matrix_mult(gam, tX,q,p,N,aux1);

///y[i+N*j]=y[i,j]? y is N X q
// cov is q X q
 for (n = 0; n < N; n++){
      sum = 0.0;
      for(j = 0; j < q; j++){
            insum = 0;
            for(i = 0; i < q; i++)
                  insum += (y[n+ N*i] - aux1[i+q*n])*cov[i+j*q];
           // Rprintf("insum[%i][%i]=%e\n", n, j, insum);

            sum += insum*(y[n+ N*j] - aux1 [j+q*n]);
            }
     delta[n] = sum;
     }
     //Rprintf("Stored information in delta\n");
...
int ginv(__CLPK_integer n, __CLPK_integer lda, double *A, double *B) {
       ...
       // Rprintf("Allocated memory in ginv\n");
...
// Rprintf("Made a diagonal matrix in B\n");

        copymx(A, n, n, lda, C);
  //      Rprintf("Coppied A into C\n");

        dgelss_(&n, &n, &NRHS, C, &n, B, &LDB, sing, &RCOND, &IRANK, WORK, &LDWORK, &info);
    //    Rprintf("Called dgelss_\n");
        if(COMMENTS && info != 0) {
      ...
      //    Rprintf("Freed and Allocated WORK");
          dgelss_(&n, &n, &NRHS, C, &n, B, &LDB, sing, &RCOND, &IRANK, WORK, &LDWORK, &info);
        //  Rprintf("Called deglss_ again\n");
...
/////////////////////////////for regression
///y[i+N*j]=y[i,j]? y is N X q
//x should be bigx so x is (N) X (p+1)
//y should be y so NXq GG =K pp =p+1
// gam should be q X (p+1)
//To print into R
// Rprintf("Print message in C format to R console");
void Gam1(int *NN, int *pp, int *qq,int *GG, double *x,double *y, double *z, int *gg, double *gam) {
  int j,k,i;
int N=*NN;
int p=*pp;
int q=*qq;
...
double *aux1 = (double*)malloc(sizeof(double)*p*p);
double *aux2 = (double*)malloc(sizeof(double)*p*q);
double *iaux1 = (double*)malloc(sizeof(double)*p*p);
//Rprintf("Allocated memory in Gam1\n");
for(i=0; i < N; i++) {
                Wt[i] = z[i + N*g];
...
for(j=0; j < p; j++) {
          for(k=0; k < p; k++) {
            //Rprintf("Writing into aux1[%i][%i]\n", j, k);
            aux1[j + k*p] = 0;
            for(i=0; i < N; i++)
               aux1[j + k*p] += Wt[i]*x[i+N*j]*x[i+N*k];

}
}

//check aux1
// Rprintf("Getting inverse of aux1\n");
ginv(p,p,aux1,iaux1);
//for(j=0; j < p; j++)
  //for(k=0; k < p; k++)
//Rprintf("Cov[%i][%i]=%f\n", j, k, iaux1[j+k*p]);
...
for(j=0; j < q; j++) {
for(k=0; k < p; k++) {
	    //Rprintf("Writing into aux2[%i][%i]\n", j, k);
            aux2[j + k*q] = 0;
            for(i=0; i < N; i++)
               aux2[j + k*q] += Wt[i]*y[i+N*j]*x[i+N*k];


}
}
//check aux 2
//Rprintf("Multiplying aux2 and iaux1\n");
matrix_mult(aux2, iaux1,q,p,p,gam);
...
//To print into R
// Rprintf("Print message in C format to R console");
void CovarianceY(int *NN, int *pp, int *qq,int *GG, double *x,double *y, double *z,double *gam, int *gg, double *Sigma) {
  int j,k,i;
int N=*NN;
int p=*pp;
int q=*qq;
...
double *Wt = (double*)malloc(sizeof(double)*N);
double *aux1 = (double*)malloc(sizeof(double)*q*N);
double *aux2 = (double*)malloc(sizeof(double)*q*N);
double *tX = (double*)malloc(sizeof(double)*p*N);
//Rprintf("Allocated Memory in CovarianceY\n");

//Rprintf("Transposing X into tX");
transpose(x,NN,pp,tX);
sum=0;
for(i=0; i < N; i++) {
                Wt[i] = z[i + N*g];
...
//aux 1 is q X N
//Rprintf("Multiplying gam and tX into aux1\n");
matrix_mult(gam, tX,q,p,N,aux1);
for(j=0; j < q; j++)
  for(i=0; i < N; i++)
  {
    aux2[i+N*j]=y[i+N*j]-aux1[j+q*i];
    if (aux2[i+N*j]<1e-6) aux2[i+N*j]=1e-6;
    //Rprintf("W[%i]=%e, trbsom for cluster=%i,treb[%i][%i]=%e\n",i,Wt[i], g,j, i, aux2[i+N*j]);
  }
  //Rprintf("Writing into Sigma\n");
for(j=0; j < q; j++) {
          for(k=0; k < q; k++) {
            Sigma[j + k*q] = 0;
            for(i=0; i < N; i++)
               Sigma[j + k*q] += Wt[i]*aux2[i+N*j]*aux2[i+N*k];
                // (y[i+N*j]-aux1[j+q*i])*(y[i+N*k]-aux1[k+q*i]);
		Sigma[j + k*q]/=sum;
	//	Rprintf("sum=%e,Sigma[%i][%i]=%e\n", sum,j, k, Sigma[j+k*q]);
...
/////////////////////////////for regression
///y[i+N*j]=y[i,j]? y is N X q
void Covariance1(int N, int p, int q,int G, double *x,double *y, double *z, int g, double *coef) {
  int j,k,i;
        double sum;
        double *Wt = (double*)malloc(sizeof(double)*N);
  ...
  sum=0;
        ...
        for(j=0; j < q; j++) {
          for(k=0; k < p; k++) {
                        ...
                        coef[j + k*p] = 0;
                for(i=0; i < N; i++) {
                coef[j + k*q] += Wt[i]*y[i+N*j]*x[i+N*k];
